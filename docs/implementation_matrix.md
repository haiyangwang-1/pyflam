...
| `rskelf` | Compact recursive skeletonization elimination factors | Compact `rskelf_mv`, `rskelf_sv`, Cholesky helpers, partial-factor helpers, and logdet | Selected inversion for complete factors; compact identity-RHS fallback for partial factors | Symmetry modes, complex, proxy, stop functions, transpose/adjoint solves, diagonal/spdiag modes | Partial factors use compact sweeps where selected inversion is not defined |
